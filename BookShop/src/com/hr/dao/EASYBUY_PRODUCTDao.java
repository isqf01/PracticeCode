...
public class EASYBUY_PRODUCTDao {
	...
	public static int totalPage(int count){
		...
		try {
			...
			rs = ps.executeQuery();
			if(rs.next()){
				int sum = rs.getInt(1);//获取表的总行数
				if(sum%count==0){
					tpage = sum/count;//总行数是每一页行数的整数倍
				}else {
					tpage = sum/count+1;//不是整数倍加一
				}
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public static ArrayList<EASYBUY_PRODUCT> selectAllByT(){
		...
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sql = "select * from EASYBUY_PRODUCT order by EP_PRICE asc limit 0,8";
		try {
			ps = conn.prepareStatement(sql);
